...
- Content identity: `algorithm`, `digest`, `scope`, and `scheme`.
- Locator: `type` and `value` (for example, a filesystem path).

The locator is never used as the content digest. `scheme=complete` supports a
complete SHA-256 digest. `scheme=fast` is explicitly limited to discovery and
development change detection and is not a production certification identity.
RAH-1 does not implement automatic relocation or rebinding.
...
Binding. It is serialized with sorted object keys and compact JSON, then
hashed with SHA-256 as `execution_binding_fingerprint`.
...
`HIGH` effort and token budget. Trace v2 keeps requested, Studio-resolved, and
Foundation/Adapter-effective thinking distinct. If an adapter cannot represent
an explicit effort or budget, Foundation returns an explicit unsupported
resolution rather than silently downgrading it.
